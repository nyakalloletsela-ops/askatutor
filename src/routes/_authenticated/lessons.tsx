...
import {
  ...
  rescheduleSession,
  completeSession,
} from "@/application/use-cases/sessions/manage-session";
...
function LessonsPage() {
  ...
  const upcoming = rows.filter(
    (r) =>
      r.status === "scheduled" &&
      new Date(r.scheduled_at).getTime() >= now - r.duration_min * 60000,
  );
    ...
    list.length === 0 ? (
      ...
      <div className="space-y-2">
        {list.map((l) => {
          ...
          const joinable =
            l.status === "scheduled" &&
            startsAt.getTime() - now < 10 * 60000 &&
            now - startsAt.getTime() < l.duration_min * 60000;
            ...
            <Card key={l.id}>
              <CardContent className="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  {l.cancel_reason && (
                    <p className="text-xs text-muted-foreground">Reason: {l.cancel_reason}</p>
                  )}
                </div>
                {l.status === "scheduled" && (
  ...
  return (
    <PageContainer
      title="My lessons"
      description="All your scheduled, past, and cancelled sessions."
    >
      ...
      <Dialog open={!!resched} onOpenChange={(o) => !o && setResched(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Pick a new start time. The tutor's availability and buffers will be checked.
            </DialogDescription>
          </DialogHeader>
          <Input
            type="datetime-local"
