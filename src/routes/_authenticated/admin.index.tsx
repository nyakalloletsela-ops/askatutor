...
import {
  ...
  adminListUsers,
  adminDeleteUser,
} from "@/application/use-cases/admin/user-management";
import { sendSubscriptionDecisionEmail } from "@/application/use-cases/support/help";
...
function SubjectsManager() {
  ...
  const grouped: Record<SubjectRow["level"], SubjectRow[]> = {
    primary: [],
    high_school: [],
    tertiary: [],
  };
...
function TutorCoursesQueue() {
  ...
  const load = async () => {
    ...
    if (ids.length) {
      ...
      (pr ?? []).forEach(
        (p) => (map[p.id] = (p as { id: string; full_name: string | null }).full_name ?? "Unnamed"),
      );
      setNames(map);
    }
  };
  ...
  const setStatus = async (row: CourseRow, status: "approved" | "rejected") => {
    ...
    if (status === "approved") {
      ...
      await supabase
        .from("subjects")
        .insert({ name: row.name, level: row.level })
        .then(() => {});
      ...
      if (!current.includes(row.name)) {
        await supabase
          .from("profiles")
          .update({ subjects: [...current, row.name] })
          .eq("id", row.tutor_id);
      }
    }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Tutor course proposals · {pending.length} pending</CardTitle>
      </CardHeader>
      <CardContent>
        {pending.length === 0 ? (
...
function TutorApplicationsQueue() {
  ...
  const visible =
    filter === "pending"
      ? rows.filter((r) => r.status === "pending" || r.status === "needs_info")
      : rows;
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle>
          Tutor applications · {rows.filter((r) => r.status === "pending").length} pending
        </CardTitle>
          ...
          <ul className="space-y-4">
            {visible.map((r) => (
              <li key={r.id} className="rounded-lg border p-4">
                ...
                <p className="text-sm">
                  <span className="font-medium">Bio:</span> {r.bio}
                </p>
                <p className="mt-1 text-sm">
                  <span className="font-medium">Qualifications:</span> {r.qualifications}
                </p>
                <div className="mt-3">
