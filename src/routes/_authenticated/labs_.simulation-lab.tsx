  ...
  head: () => ({
    meta: [
      { title: "Simulation Lab — Ask A Tutor Live" },
      {
        name: "description",
        content:
          "Turn any learning prompt into a live 3D simulation. Save and remix your reusable knowledge universe.",
      },
    ],
  }),
...
function Lab3DPage() {
  ...
  async function refreshLibrary() {
    try {
      const { simulations } = await listFn({
  ...
  useEffect(() => {
    refreshLibrary(); /* eslint-disable-next-line */
  }, []);
  useEffect(() => {
    refreshLibrary(); /* eslint-disable-next-line */
  }, [subject]);
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden md:grid-cols-[300px_1fr_320px]">
        ...
        <aside className="hidden flex-col gap-3 border-r border-white/5 bg-black/30 p-4 md:flex">
          ...
          <p className="text-xs text-white/60">
            Type any question, scenario, or statement from{" "}
            <span className="text-white">any subject</span> — physics, biology, chemistry, math,
            economics, history, language — and the AI builds a live 2D/3D simulation showing motion,
            structure, and labelled detail.
          </p>
          <Textarea
            value={prompt}
        ...
        <main ref={sceneWrapRef} className="relative h-full min-h-0 w-full overflow-hidden">
          <div className="absolute inset-0">
            <SimDispatch
              ...
              resetKey={resetKey}
              timeScale={timeScale}
              onCanvasReady={(gl: THREE.WebGLRenderer) => {
                glRef.current = gl;
              }}
              ...
              <div className="mb-2 flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-sm font-semibold">
                    {schema.objects[selectedObj].label ?? schema.objects[selectedObj].type}
                  </div>
                </div>
                <button
                ...
                if (!ex)
                  return (
                    <div className="text-xs text-white/60">
                      No explanation provided. Click another object or generate a new simulation.
                    </div>
                  );
                return (
                  <div className="space-y-2 text-xs">
                    {ex.definition && (
                      <div>
                        ...
                        <div className="text-white/80">{ex.definition}</div>
                      </div>
                    )}
                    {ex.purpose && (
                      <div>
                        ...
                        <div className="text-white/80">{ex.purpose}</div>
                      </div>
                    )}
                    {ex.keyFacts && ex.keyFacts.length > 0 && (
                      <div>
                        ...
                        <ul className="ml-3 list-disc text-white/80">
                          {ex.keyFacts.map((f, i) => (
                            <li key={i}>{f}</li>
                          ))}
                        </ul>
                    ...
                    {ex.misconceptions && ex.misconceptions.length > 0 && (
                      <div>
                        ...
                        <ul className="ml-3 list-disc text-rose-200/80">
                          {ex.misconceptions.map((f, i) => (
                            <li key={i}>{f}</li>
                          ))}
                        </ul>
                      </div>
        ...
        <aside className="hidden flex-col border-l border-white/5 bg-black/30 md:flex">
          <Tabs
            value={rightTab}
            onValueChange={(v) => setRightTab(v as "library" | "chat")}
              ...
              <div className="flex-1 overflow-y-auto p-2">
                {libraryError ? (
                  ...
                  library.map((it) => (
                    <div
                      key={it.id}
                      className="group mb-2 flex gap-2 rounded border border-white/10 bg-black/30 p-2 hover:border-violet-400/50"
                    >
                      <button
      ...
      <Dialog open={!!match} onOpenChange={(o) => !o && setMatch(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              "{match?.title}" looks similar to your prompt. Load it, or generate a brand-new
              simulation?
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <DialogContent className="max-w-lg">
              ...
              return (
                <div className="space-y-3">
                  <div className="text-xs text-white/50">
                    Question {quizIdx + 1} of {schema.quiz.length} ·{" "}
                    <span className="uppercase">{q.difficulty}</span> · {q.type}
                  </div>
                  ...
                  {q.type === "mcq" && q.options ? (
                    ...
                    <div className="flex gap-2">
                      {["True", "False"].map((opt) => (
                        <button
                          key={opt}
                          onClick={() => setQuizAnswers({ ...quizAnswers, [quizIdx]: opt })}
                          className={`flex-1 rounded border px-3 py-2 text-sm ${userAns === opt ? "border-violet-400 bg-violet-500/20" : "border-white/10"}`}
                        >
                          {opt}
                        </button>
                      ))}
                    ...
                    <Input
                      value={userAns ?? ""}
                      onChange={(e) =>
                        setQuizAnswers({ ...quizAnswers, [quizIdx]: e.target.value })
                      }
                  ...
                  {quizReveal && (
                    <div
                      className={`rounded border p-2 text-sm ${correct ? "border-emerald-400/40 bg-emerald-500/10 text-emerald-200" : "border-rose-400/40 bg-rose-500/10 text-rose-200"}`}
                    >
          ...
          <DialogFooter className="gap-2">
            ...
            {!quizReveal ? (
              <Button onClick={() => setQuizReveal(true)} disabled={!quizAnswers[quizIdx]}>
                Check
              </Button>
