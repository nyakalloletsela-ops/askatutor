/**
 * Multi-agent registry (Application-owned). Each role is a self-contained AI
 * persona with its own model, system prompt, and output contract. All agents
 * share one rule: output MUST be markdown, math MUST be wrapped in $...$ /
 * $$...$$ for KaTeX, and diagrams MUST be in ```mermaid fenced code blocks.
 * The renderer (<SmartMarkdown/>) understands both, so any caller can render
 * any agent.
 *
 * Adding a new model provider (e.g. self-hosted Ollama) means adding ONE entry
 * here — no caller changes needed.
 *
 * This is static configuration consumed by the AI use cases; it is NOT an
