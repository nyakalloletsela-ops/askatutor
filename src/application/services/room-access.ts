/**
 * Room membership resolution — fail-closed classroom access logic.
 *
 * A user may enter a room only if a real session exists with that room_id and
 * the user is the tutor, the student, or an admin. There is no demo/bypass
 * path, and any lookup error propagates so callers can deny access rather than
 * allow it. Role (tutor/admin) is NEVER derived from the client.
 */
...
export interface RoomLookup {
  ...
  userId: string;
  isAdmin: boolean;
  findSessionByRoom(roomId: string): Promise<{
