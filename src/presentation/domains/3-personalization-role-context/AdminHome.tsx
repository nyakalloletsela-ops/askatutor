...
import { Checkbox } from "../8-core-ux-navigation/ui/checkbox";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  ...
  AlertDialogHeader,
  AlertDialogTitle,
} from "../8-core-ux-navigation/ui/alert-dialog";
import {
  Users,
  GraduationCap,
  Calendar,
...
export function AdminHome({ firstName }: { firstName: string }) {
  ...
  const approveApp = useServerFn(approveTutorApplication);
  const rejectApp = useServerFn(rejectTutorApplication);
  const logDecision = useServerFn(logTutorDecision);

  const load = async () => {
    const result = await loadDashboard();
    setApps((result.applications as AppRow[]) ?? []);
    setTutors(((result.tutors as TutorProfile[]) ?? []).slice(0, 30));
    setSessions((result.sessions as SessionLite[]) ?? []);
    setCounts(result.counts);
    setAudit((result.auditLog as AuditRow[]) ?? []);
  ...
  const decide = async (row: AppRow, approve: boolean) => {
    ...
    try {
      if (approve) {
        await approveApp({ data: { applicationId: row.id, notes: notes[row.id] } });
      } else {
        await rejectApp({ data: { applicationId: row.id, notes: notes[row.id] } });
      }
      await logDecision({
        data: {
