...
export function LanguageView({ schema }: Props) {
  ...
  if (!lang)
    return (
      <div className="flex h-full items-center justify-center text-white/50">No language scene</div>
    );
  ...
  return (
    <div className="flex h-full w-full flex-col bg-gradient-to-b from-[#0b1020] to-[#1a0b2e] p-4 text-white">
      ...
      <div className="mb-3 rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-xs text-white/70">
        📍 {lang.setting}
      </div>
        ...
        {lang.dialogue.map((line, i) => {
          ...
          return (
            <div key={i} className={`flex gap-2 ${isMe ? "flex-row-reverse" : "flex-row"}`}>
              ...
              <div
                className={`max-w-[75%] rounded-2xl px-4 py-2 ${isMe ? "bg-violet-500/30 border border-violet-400/40" : "bg-white/10 border border-white/10"}`}
              >
                ...
                <div className="flex items-center gap-2 text-base">
                  ...
                  <button
                    onClick={() => speak(line.original, line.lang)}
                ...
                {line.translation && (
                  <div className="mt-1 text-xs italic text-white/60">{line.translation}</div>
                )}
              </div>
            </div>
          );
