...
export function Scene2D({ schema, playing, resetKey, timeScale }: Props) {
  ...
  if (!g) {
    return (
      <div className="flex h-full items-center justify-center text-white/50">No 2D graph data</div>
    );
  }
  ...
  return (
    <div className="flex h-full w-full items-center justify-center bg-[#0b1020] p-4">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-full w-full max-w-5xl">
        ...
        <text x={W - PAD + 6} y={sy(0) + 4} fill="#94a3b8" fontSize="14">
          {g.xLabel ?? "x"}
        </text>
        <text x={sx(0) + 6} y={PAD - 6} fill="#94a3b8" fontSize="14">
          {g.yLabel ?? "y"}
        </text>
        {/* curves */}
        {curvePaths.map((c, i) => (
        ...
        {g.points.map((p, i) => (
          <g key={i}>
            ...
            {p.label && (
              <text x={sx(p.x) + 8} y={sy(p.y) - 6} fill="#e2e8f0" fontSize="11">
                {p.label}
              </text>
            )}
          </g>
        ))}
        <text x={PAD} y={PAD - 16} fill="#cbd5e1" fontSize="14" fontWeight="600">
          {schema.title}
        </text>
        <text x={W - PAD} y={H - 12} textAnchor="end" fill="#64748b" fontSize="11">
          t = {t.toFixed(2)}s
        </text>
      </svg>
    </div>
