...
export function GeoView({ schema }: Props) {
  const g = schema.geo;
  if (!g)
    return <div className="flex h-full items-center justify-center text-white/50">No geo data</div>;
  const W = 1000,
    H = 500;
            ...
            return (
              <g key={`pin-${i}`}>
                ...
                <circle
                  ...
                  strokeWidth="1"
                />
                <text x={x + 8} y={y - 8} fill="#e2e8f0" fontSize="12" fontWeight="600">
                  {r.label ?? r.name}
                </text>
                {r.value !== undefined && (
                  <text x={x + 8} y={y + 6} fill="#94a3b8" fontSize="10">
                    {r.value}
                  </text>
                )}
              </g>
