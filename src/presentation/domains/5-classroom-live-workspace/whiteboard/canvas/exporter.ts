...
function bounds(shapes: Shape[]) {
  ...
  for (const s of shapes) {
    ...
    if (s.type === "pencil" || s.type === "highlighter") {
      ...
      const xmn = Math.min(s.x1, s.x2),
        xmx = Math.max(s.x1, s.x2);
      const ymn = Math.min(s.y1, s.y2),
        ymx = Math.max(s.y1, s.y2);
      const p = s.size + 8;
      b = { x: xmn - p, y: ymn - p, w: xmx - xmn + p * 2, h: ymx - ymn + p * 2 };
    } else {
...
async function renderToCanvas(
  ...
  render({
    ctx,
    shapes,
    camera: { x: -b.x * scale, y: -b.y * scale, z: scale },
    width: canvas.width,
...
export function exportJSON(shapes: Shape[], filename = "whiteboard.json") {
  const blob = new Blob([JSON.stringify({ version: 1, shapes }, null, 2)], {
