...
const MATH_RE = /\$\$([\s\S]+?)\$\$/g;
const LATEX_HINT_RE =
  /\\(?:int|iint|iiint|oint|frac|sqrt|sum|prod|lim|begin|partial|nabla|vec|sin|cos|tan|log|ln|alpha|beta|gamma|theta|pi|infty|leq|geq|neq|rightarrow)|\b(?:int|iint|iiint|sqrt|sum|prod|lim|frac|partial)(?=_|\b)|[∫∬∭∮∑∏√∞≈≤≥≠±∂∇πθΔ]/i;
...
function classifyTextualBlock(text: string): ConvertedBlock {
  ...
  if (singleLine && looksLikeMathLine(cleaned))
    return { kind: "math", latex: normalizeLatex(cleaned) };
...
export function blocksToShapes(
    ...
    if (b.kind === "svg") {
      ...
      for (const lb of labels) {
        ...
        shapes.push({
          ...
          text: lb.text,
          color: lb.color,
          size: Math.max(10, Math.round(lb.size * scale)),
          z: z++,
          page: 1,
          ts: tick(),
        });
