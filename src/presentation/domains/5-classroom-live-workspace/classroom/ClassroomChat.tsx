...
import { sendClassroomMessage } from "@/application/use-cases/classroom/classroom-chat";
...
export function ClassroomChat({ roomId, userId, displayName }: Props) {
  ...
  const sendMessageFn = useServerFn(sendClassroomMessage);
  ...
  const send = async () => {
    ...
    setSending(true);
    try {
      await sendMessageFn({ data: { roomId, displayName, body } });
      setInput("");
    } catch (e) {
