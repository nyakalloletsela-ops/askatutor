      ...
      <div className="pointer-events-auto mx-auto flex w-full max-w-md items-center justify-center gap-1.5 rounded-2xl border border-white/10 bg-zinc-900/90 px-2 py-1.5 shadow-2xl backdrop-blur">
        ...
        <Popover>
          ...
          <PopoverContent
            side="top"
            align="end"
            className="w-64 rounded-2xl border-white/10 bg-zinc-900/95 p-3 text-white backdrop-blur"
          >
            <Group title="Share">
              <PillButton
            ...
            <Group title="Video layout">
              <PillButton
                active={layoutMode === "TOP_STRIP"}
                onClick={() => onSetLayout("TOP_STRIP")}
                icon={<LayoutPanelTop className="h-3.5 w-3.5" />}
                label="Top strip"
              />
              <PillButton
                active={layoutMode === "DOCKED"}
            ...
            <Group title="Main stage">
              <PillButton
                ...
                onClick={() => onSetStage("whiteboard")}
                icon={<PenTool className="h-3.5 w-3.5" />}
                label="Whiteboard"
              />
              <PillButton
                active={stage === "phet"}
                onClick={() => onSetStage("phet")}
                icon={<FlaskConical className="h-3.5 w-3.5" />}
                label="PhET Lab"
              />
              <PillButton
                active={stage === "sim"}
                onClick={() => onSetStage("sim")}
        ...
        <Button
          ...
          variant="destructive"
          size="sm"
          className="ml-1 h-9 shrink-0 rounded-full px-3"
        >
  ...
  return (
    <div className="pointer-events-auto mx-auto flex w-full max-w-3xl items-center justify-center gap-1.5 rounded-2xl border border-white/10 bg-zinc-900/85 px-2 py-2 shadow-2xl backdrop-blur sm:gap-2">
      ...
      <Button
        ...
        variant="destructive"
        size="sm"
        className="ml-1 h-9 rounded-full px-3"
      >
...
function Group({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-2 last:mb-0">
      <div className="mb-1 px-1 text-[10px] font-medium uppercase tracking-wider text-white/50">
        {title}
      </div>
