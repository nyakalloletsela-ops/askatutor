...
export function ClassroomHeader({
  ...
  void roomId;
  const subtitle = [tutorName && `Tutor: ${tutorName}`, studentName && `Student: ${studentName}`]
    .filter(Boolean)
    .join(" · ");
  ...
  return (
    <header className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b bg-background/95 px-3 py-2 backdrop-blur sm:px-4">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span
          className={`hidden items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-medium sm:inline-flex ${recording ? "border-rose-500/40 bg-rose-500/10 text-rose-600" : "bg-card text-muted-foreground"}`}
        >
          <Circle
            className={`h-2 w-2 fill-current ${recording ? "animate-pulse text-rose-500" : ""}`}
          />
          {recording ? "Recording" : "Not recording"}
        </span>
        <div className="hidden sm:block">
          <QualityChip q={quality} />
        </div>
