    ...
    <section className="mx-auto max-w-6xl px-4 py-14 md:py-20">
      <div className="mb-7 flex items-end justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold tracking-tight md:text-3xl">
            Learn from someone who fits your goal.
          </h2>
        </div>
        <Button asChild variant="ghost" className="hidden sm:inline-flex">
          <Link to="/tutors">
            See all tutors <ArrowRight className="ml-1 h-4 w-4" />
          </Link>
        </Button>
      </div>
      <div className="grid gap-4 md:grid-cols-3">
        {tutors.map((tutor) => (
          <div
            key={tutor.id}
            className="rounded-2xl border border-border/60 bg-card p-5 transition hover:-translate-y-0.5 hover:shadow-md"
          >
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {tutor.review_count
                    ? `${Number(tutor.avg_rating ?? 0).toFixed(1)} · ${tutor.review_count} reviews`
                    : "New tutor"}
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 flex items-center justify-between gap-3">
              ...
              <Button asChild size="sm" className="bg-aurora text-white">
                <Link to="/tutor/$id" params={{ id: tutor.id }}>
                  View tutor
                </Link>
              </Button>
            </div>
...
const learningTools = [
  {
    icon: CheckCircle2,
    title: "Live tutoring",
    text: "Personal help when a concept needs a human.",
  },
        ...
        <div className="grid gap-5 md:grid-cols-3">
          <div className="rounded-3xl border border-border/60 bg-card p-7 md:col-span-2 md:p-9">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">
              Use live tutoring alongside lessons, practice, resources and feedback so each session
          ...
          <div className="rounded-3xl border border-primary/20 bg-primary/5 p-7 md:p-9">
            ...
            <p className="mt-3 text-sm leading-6 text-muted-foreground">
              Get explanations, guidance and practice support without losing the human tutor at the
              centre of learning.
            </p>
            <Button asChild variant="outline" className="mt-6 rounded-xl">
              <Link to="/ai-tools">
                Explore AI support <ArrowRight className="ml-1 h-4 w-4" />
              </Link>
            </Button>
          </div>
        </div>
    ...
    <section className="mx-auto max-w-6xl px-4 py-14 md:py-20">
      <div className="rounded-3xl border border-border/60 bg-card p-7 md:flex md:items-center md:justify-between md:gap-8 md:p-10">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold tracking-tight md:text-3xl">
            Teach what you know. Reach more learners.
          </h2>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">
            Create your tutor profile, set your availability and connect with learners who need your
            expertise.
          </p>
        </div>
        <Button asChild size="lg" variant="outline" className="mt-6 shrink-0 rounded-2xl md:mt-0">
          <Link to="/become-tutor">
            <Users className="mr-2 h-5 w-5" /> Become a Tutor
          </Link>
        </Button>
      </div>
    </section>
