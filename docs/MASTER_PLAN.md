...
- **Status:** IN_PROGRESS (control system installed and AT-0001 COMPLETED; AT-0002 PARTIALLY VERIFIED — non-prod migration reproduction VERIFIED 73/73 session 4; production applied-state PARTIALLY VERIFIED (historical narrative); cross-learner runtime isolation NOT VERIFIED — see Work Item Register / capability entry)
...
- **`AT-0002` — Live Database Verification & RLS Isolation Confirmation** (`PARTIALLY VERIFIED`. Production applied-state: PARTIALLY VERIFIED via source/migration RLS evidence + recorded session-3 live narrative (historical, no captured in-repo artifacts). Session 4: **non-production migration reproduction VERIFIED** — the repository's 73 migrations applied 73/73 to dedicated hosted non-prod `askatutorlive-at0002-nonprod` (ref `rwpxaejhouunxlcibpou`), schema/RLS reconciled with captured evidence (see capability entry). **Definitive cross-learner A/B runtime isolation still NOT VERIFIED / BLOCKED** — requires two learner identities + controlled records (non-prod `rwpxaejhouunxlcibpou` now exists as an authorized candidate environment) and explicit resume authorization. See capability entry below).
...
- **Verification level:** Production applied-state (Set A: applied schema + RLS policy definitions + row-presence) = `PARTIALLY VERIFIED` — source/migration RLS evidence plus recorded session-3 live narrative (no captured in-repo artifacts). **Non-production migration reproduction (session 4) = `VERIFIED`** — 73/73 migrations applied to `rwpxaejhouunxlcibpou` with captured evidence (61 RLS-enabled tables incl. Set A/B, enums, functions/ACLs, auth baseline: email-confirmation-on, 0 users, 0 objects; applied-history = exactly the 73 repo filenames). Cross-learner runtime isolation and application-boundary runtime behavior = `NOT VERIFIED`. `PRODUCTION-VERIFIED` is **not** claimed for either environment: production results are session-3 narrative; non-prod is a non-production environment.
- **Remaining uncertainty:** Cross-learner (A vs B) **runtime enforcement behavior** — whether one authenticated learner can actually SELECT/UPDATE/DELETE another learner's rows, or INSERT falsely-attributed rows — remains `UNKNOWN — REQUIRES VERIFICATION` (unexercised: empty production DB + no identities). The non-prod project `rwpxaejhouunxlcibpou` now exists as an isolated candidate environment for this test once two learner identities + controlled records are authorized.
- **Dependencies:** An authorized, populated test environment with **two authenticated learner identities (Learner A / B)** and controlled A/B-owned records (or explicit authorization to create test data in a non-production environment — the dedicated non-prod project `rwpxaejhouunxlcibpou` is the ready candidate) to execute the definitive cross-learner RLS tests (read + write + IDOR). Explicit re-authorization to resume.
