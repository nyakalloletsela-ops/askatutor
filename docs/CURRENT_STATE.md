...
- **AT-0002 outcome:** Substantially verified (session 3, live production, historically recorded — see AT-0002 Evidence Classification below): the operator-authenticated Supabase CLI/Management API provides a working HTTPS path that bypasses the raw 5432 port, enabling **read-only live applied-database verification**. Applied schema, RLS policy definitions, helper functions, `app_role` enum, and empty-table/empty-user state for the 5 learner tables are recorded as **verified live against production (session 3, historical narrative)** for Set A below, with the **source↔applied reconciliation recorded as MATCH** for all 5. These are narrative evidence from a prior session; no captured query-result artifacts are stored in this repository. The **definitive cross-learner (A vs B) runtime isolation test remains NOT VERIFIED / not executable** because the production DB has **zero users** (`auth.users`=0) and **zero rows** in all 5 learner tables, and creating test identities/data would be out-of-scope production modification. AT-0002 = `PARTIALLY VERIFIED` (static/applied RLS model PARTIALLY VERIFIED via source+recorded live evidence; current reproducible live evidence NOT VERIFIED; cross-learner runtime isolation NOT VERIFIED). Not COMPLETED, not fully VERIFIED. See Isolation Findings and AT-0002 Evidence Classification below.
...
- **Distinction preserved:** source/migration evidence (migration files) vs historical live verification (session-3 narrative) vs captured evidence (none stored) vs runtime verification (none executed). Only the migration files are verifiable from the repository alone today.
