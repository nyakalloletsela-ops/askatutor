# AskATutorLive — Sanitized Architecture Inventory

> Generated locally with PowerShell.
> This document contains repository metadata only.
> Secret files, credentials, vendor directories, build output and Git metadata were excluded.
...
| `src/integrations/supabase/auth-middleware.ts` | `.ts` | 2.3 KB | Authentication/Authorization |
...
Only package names and versions are included. No environment variables or secrets are exported.
...
Migration filenames are listed for architectural sequencing. SQL contents are NOT included.
...
## 8. Configuration Files Detected
...
This inventory describes repository structure and metadata. It does not claim to know the actual responsibility of every file.
A separate architecture-analysis step should infer file responsibilities from names, paths, imports and other non-secret metadata.
